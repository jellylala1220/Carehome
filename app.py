        ...
        if st.button("Start Batch Prediction"):
            ...
            if not valid_care_homes:
                ...
                for i, care_home_id in enumerate(valid_care_homes):
                    ...
                    pred_df_single, target_month = predict_next_month_bayesian(df_carehome)
    ...
    if st.session_state.get('df') is None:
        ...
        if upload_pred_file:
            try:
                ...
                with col1:
                    selected_ch_id = st.selectbox("Filter by Care Home", options=["All"] + sorted(list(ch_map.keys())), format_func=lambda x: "All (for score comparison)" if x == "All" else f"{ch_map.get(x, 'Unknown')} ({x})")
                with col2:
                    selected_score = st.selectbox("Filter by NEWS2 Score (for 'All' view)", options=list(range(11)), index=1, disabled=(selected_ch_id != "All"))
                ...
                if selected_ch_id != "All":
                    ...
                    if not ch_pred.empty:
                        ...
                        for _, row in ch_pred.iterrows():
                            fig.add_trace(go.Scatter(x=[row['Month']], y=[row['Predicted Mean']], mode='markers', error_y=dict(type='data', symmetric=False, array=[row['95% Upper'] - row['Predicted Mean']], arrayminus=[row['Predicted Mean'] - row['95% Lower']]), name=f"Pred Score {int(row['NEWS2 Score'])}", marker=dict(size=10)))
                    ...
                    if not comp_df.empty:
                        ...
                        fig_bar = go.Figure()
                        fig_bar.add_trace(go.Bar(x=comp_df['Care Home Name'], y=comp_df['Last Month Actual'], name=f'Actual Count ({last_month})'))
                        ...
                        fig_bar.update_layout(barmode='group', title=f"Actual vs. Predicted for Score {selected_score}")
...
# ==============================================================================
    ...
    if st.session_state.get('df') is not None:
        ...
        with tab1:
            st.subheader("Monthly High-Risk Event Heatmap (NEWS2 >= 6)")
            if not details_df.empty:
                ...
                fig_heatmap = px.imshow(heatmap_data, labels=dict(x="Month", y="Care Home", color="Events"), color_continuous_scale=px.colors.sequential.Reds, height=heatmap_height)
        ...
        with tab2:
            ...
            if 'Latitude' in df.columns and 'Longitude' in df.columns and not summary_df.empty:
                ...
                if not geospatial_df.empty:
                    ...
                    fig_map = px.scatter_map(geospatial_df, lat="Latitude", lon="Longitude", hover_name="Care Home Name", hover_data={"pi": ":.2%"}, size="size", color="pi", color_continuous_scale=px.colors.sequential.Reds, zoom=5, height=600, map_style="open-street-map")
